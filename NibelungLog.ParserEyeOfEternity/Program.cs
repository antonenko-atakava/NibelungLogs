...
if (string.IsNullOrEmpty(logPath))
{
    ...
    if (isLinux)
        logPath = "/var/log/nibelunglog/parser-eye-of-eternity.log";
    else
    {
        var appDirectory = AppDomain.CurrentDomain.BaseDirectory;
        var logsDirectory = Path.Combine(appDirectory, "logs");
        logPath = Path.Combine(logsDirectory, "parser-eye-of-eternity.log");
    }
}
...
logger.LogInformation("Запуск парсера рейдов Око вечности");
logger.LogInformation("═══════════════════════════════════════════════════════════");
