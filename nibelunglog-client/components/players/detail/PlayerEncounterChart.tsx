...
export function PlayerEncounterChart({ playerId, specStatistics = [] }: PlayerEncounterChartProps) {
  ...
  }, [isFiltersOpen, selectedSpec, selectedRaidTypeId, selectedEncounterName, startDate, endDate]);
  ...
  const chartData = useMemo(() => {
    ...
    const labels = encounters.map(e => formatDate(e.startTime));
    const dpsData = encounters.map(e => Math.round(e.dps));
    const hpsData = isHealer ? encounters.map(e => e.hps ? Math.round(e.hps) : null) : null;
