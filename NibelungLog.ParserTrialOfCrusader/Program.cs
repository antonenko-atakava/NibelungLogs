...
if (string.IsNullOrEmpty(logPath))
{
    ...
    if (isLinux)
        logPath = "/var/log/nibelunglog/parser-trial-of-crusader.log";
    else
    {
        var appDirectory = AppDomain.CurrentDomain.BaseDirectory;
        var logsDirectory = Path.Combine(appDirectory, "logs");
        logPath = Path.Combine(logsDirectory, "parser-trial-of-crusader.log");
    }
}
...
logger.LogInformation("Запуск парсера рейдов Испытание Крестоносца");
logger.LogInformation("═══════════════════════════════════════════════════════════");
...
var result = await authService.LoginAsync("melloooooo", "maximum916", 5);
...
logger.LogInformation("Поиск рейдов Испытание Крестоносца...");
var raids = await authService.GetRaidsByMapAsync(5, "649", 1);
